...
def load_weights(
    pretrained_checkpoint_path: str, target_model: torch.nn.Module, only_rnn: bool = True
) -> torch.nn.Module:
...
def load_rnn_weights(pretrained_state_dict):
    rnn_state_dict = {
        weight_name: weight
...
def check_inclusive_keys(main_dict, sub_dict):
    for key in sub_dict:
